...
export function Filters({ filters, options, homeDirectory, dataset, searchRef, onChange, onClear }: {
  ...
  return <>
    <div className="filters flex flex-wrap items-center gap-2 px-4 pt-5 pb-4 sm:px-6">
      ...
      <Select className={selectClass} id="status" aria-label="Filter by status" value={filters.status} onChange={(event) => onChange("status", event.currentTarget.value)}><option value="all">All statuses</option><option value="completed">{dataset?.harness === "opencode" && dataset.apiVersion === 2 ? "Completed" : "Result received"}</option>{dataset?.harness === "opencode" && dataset.apiVersion === 2 && <option value="failed">Failed</option>}<option value="awaiting">Awaiting result</option></Select>
      ...
      {dataset?.harness === "opencode" && dataset.apiVersion === 2 && <Select className={selectClass} id="agent" aria-label="Filter by agent" value={filters.agent} onChange={(event) => onChange("agent", event.currentTarget.value)}><option value="all">All agents</option>{options.agent.map((agent) => <option key={agent} value={agent}>{agent}</option>)}</Select>}
